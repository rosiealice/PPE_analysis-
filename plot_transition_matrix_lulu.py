...
fig.subplots_adjust(left=0.12, right=0.90, wspace=0.25)
...
with xr.open_dataset(last_file_path, decode_times=False) as ds:
    data = ds[VAR_NAME].isel(time=last_time_idx).values * SCALE_FACTOR
    ...
    if grid_type == 'ncol':
        ...
        lat_ = lat
        lon_ = lon
    # Reshape for shape (1, 25, ngrid) or (1, 25, ncol)
    print(f"Static plot: original data shape {data.shape}")
    if data.shape[0] == 1 and data.shape[1] == 25:
        ngrid = data.shape[2]
        data = data[0]  # Remove time dim, shape (25, ngrid)
        data = data.reshape(5, 5, ngrid)  # shape (5, 5, ngrid)
    elif data.shape[-1] == 25:
        # Fallback for other shapes
        ngrid = data.shape[0]
        data = data.reshape(5, 5, ngrid)
    elif data.shape[:2] == (5, 5):
        ngrid = data.shape[2]
        # Already correct shape
    else:
        print(f"Warning: Unexpected data shape for LUxLU unpacking: {data.shape}")
        ngrid = len(lon_)
    print(f"Static plot: unpacked data shape {data.shape}, ngrid={ngrid}, lon_ shape={lon_.shape}")
    if grid_type == 'ncol':
        lat_ = ds['lat'].values if 'lat' in ds else ds['grid_lat'].values
        lon_ = ds['lon'].values if 'lon' in ds else ds['grid_lon'].values
    elif grid_type == 'lndgrid':
...
fig_last, axs_last = plt.subplots(n_lu, n_lu, figsize=(4.5*n_lu, 3*n_lu),
                                 subplot_kw={'projection': None} if n_lu <= 4 else {})
cb_ax_last = fig_last.add_axes([0.94, 0.15, 0.02, 0.7])
cb_ax_patch = fig_last.add_axes([0.04, 0.15, 0.02, 0.7])
fig_last.subplots_adjust(left=0.12, right=0.90, wspace=0.25)
if n_lu == 1:
...
for j in range(n_lu):
    for i in range(n_lu):
        ...
        if i == j:
            ...
            with xr.open_dataset(last_file_path, decode_times=False) as ds_patch:
                if 'FATES_PATCHAREA_LU' in ds_patch:
                    patcharea = ds_patch['FATES_PATCHAREA_LU'].isel(time=last_time_idx).values
            ...
            if d.shape[0] != len(lon_):
                d = np.full(len(lon_), np.nan)
            norm = SymLogNorm(linthresh=1e-7, linscale=1.0, vmin=vmin, vmax=vmax)
            sc = ax.scatter(lon_, lat_, c=d, cmap='viridis', s=8, norm=norm)
            mappable = sc
        ...
        ax.set_xticks([])
        ax.set_yticks([])
fig_last.suptitle(f"LUxLU transitions - Last timestep", fontsize=24, y=0.96)
plt.tight_layout()
...
if mappable is not None:
    fig_last.colorbar(mappable, cax=cb_ax_last, orientation='vertical', label='Transition rate (%/month)', norm=norm)
...
def update(frame):
    ...
    fig.colorbar(mappable, cax=cb_ax, orientation='vertical', label='Transition rate (%/month)', norm=norm)
    # Add colorbar for diagonal (patch area) using the first diagonal mappable
    if hasattr(fig, 'cb_ax_patch'):
        cb_ax_patch = fig.cb_ax_patch
    else:
        cb_ax_patch = fig.add_axes([0.05, 0.15, 0.02, 0.7])
