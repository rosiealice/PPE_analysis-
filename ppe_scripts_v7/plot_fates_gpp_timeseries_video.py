...
# Variable names - can be a single string or a list of string s
    ...
    N_YEARS = 126 # Number of years to process (None for all years)
    FILE_INTERVAL = 6 # Process every Nth timestep (1=all, 2=every other, 3=every third, etc.)
    tint=10
...
if(case==1):
    ...
    N_YEARS = 14 # Number of years to process (None for all years)
...
if(case==2):
    ...
    SCALING_FACTORS = {var: 3600*24*365 for var in VAR_NAMES}
    N_YEARS = 59 # Number of years to process (None for all years)
...
if(case==3):
    ...
    FILE_INTERVAL = 2 # Process every Nth timestep 
    tint=3  # tie interval
    OUTPUT_FORMAT = "gif"  # "gif" or "mp4"
    UNIT_LABELS = {var: "KgC/m2/yr" for var in VAR_NAMES}
...
OUTPUT_FILE = OUTPUT_PATH+ "/fates_" + "_".join(VAR_NAMES) + "_timeseries." + file_ext
...
try:
    ...
    # Determine how many raw timesteps to scan
    # N_YEARS determines temporal coverage, FILE_INTERVAL determines sampling rate
    max_raw_timesteps = None
    if N_YEARS is not None:
        max_raw_timesteps = N_YEARS * 12
        expected_output = max_raw_timesteps // FILE_INTERVAL
        print(f"Will scan {max_raw_timesteps} timesteps ({N_YEARS} years), using every {FILE_INTERVAL} timestep → ~{expected_output} output frames")
    ...
    for i, file_path in enumerate(all_files):
        # Stop if we've scanned enough raw timesteps
        if max_raw_timesteps is not None and timestep_counter >= max_raw_timesteps:
            break
        
        # Only add timestep if it matches the interval
        if timestep_counter % FILE_INTERVAL == 0:
    ...
    def update_frame(frame):
        ...
        for idx, (VAR_NAME, member_idx) in enumerate(subplot_info):
            ...
            # Convert frame number to actual timestep accounting for FILE_INTERVAL
            actual_timestep = frame * FILE_INTERVAL
            year = actual_timestep // 12 + 1
