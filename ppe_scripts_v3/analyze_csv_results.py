...
for region in REGIONS:
    ...
    if lai_col in df_clean.columns:
        ...
        y_lai = y_et  # Fallback to ET if LAI not available
    ...
    y_lhs_pred = lr_et.predict(lhs_samples)
    ...
    if lai_col in df_clean.columns:
        ...
        y_lai_lhs_pred = y_lhs_pred  # Fallback to ET
    ...
    et_lai_within_bounds_mask = np.zeros(len(y_lai_lhs_pred), dtype=bool)
    et_only_within_bounds_mask = np.zeros(len(y_lai_lhs_pred), dtype=bool)
    ...
    if not np.isnan(obs_et_wecann) and not np.isnan(obs_et_class_val):
        ...
        if np.any(lai_within_bounds_mask):
            ...
            print(f"  Emulator samples within BOTH LAI and ET bounds: {n_within_both}/{len(y_lai_lhs_pred)}")
        
        # Samples within ET bounds only (not LAI)
        et_only_within_bounds_mask = et_within_bounds & ~lai_within_bounds_mask
        n_et_only = np.sum(et_only_within_bounds_mask)
        print(f"  Emulator samples within ET bounds only: {n_et_only}/{len(y_lai_lhs_pred)}")
    ...
    for idx, param in enumerate(params_to_use):
        ...
        # Plot emulator samples colored by predicted LAI
        scatter_emul = ax.scatter(lhs_samples[:, param_idx], y_lhs_pred, s=15, alpha=0.4,
                  c=y_lai_lhs_pred, cmap='YlGn', edgecolors='none',
                  label=f'{n_samples} emulator samples', vmin=y_lai.min(), vmax=y_lai.max())
        ...
        # Highlight samples within BOTH LAI and ET bounds (black dots)
        if np.any(et_lai_within_bounds_mask):
            ax.scatter(lhs_samples[et_lai_within_bounds_mask, param_idx],
                      ...
                      s=20, alpha=0.9, c='black', edgecolors='yellow', linewidth=0.5,
                      label=f'Within LAI & ET bounds ({np.sum(et_lai_within_bounds_mask)})', zorder=4)
        
        # Highlight samples within ET bounds only
        if np.any(et_only_within_bounds_mask):
            ax.scatter(lhs_samples[et_only_within_bounds_mask, param_idx],
                      y_lhs_pred[et_only_within_bounds_mask],
                      s=15, alpha=0.6, c='red', edgecolors='none',
                      label=f'Within ET bounds only ({np.sum(et_only_within_bounds_mask)})', zorder=3)
        
        # Plot actual ensemble members on top, colored by actual LAI
        ax.scatter(df_clean[param], y_et, s=120, alpha=0.9, c=y_lai,
                  cmap='YlGn', edgecolors='black', linewidth=2, 
                  label='Ensemble members', zorder=5,
                  vmin=y_lai.min(), vmax=y_lai.max())
        
        # Add observational ET as horizontal lines
        if not np.isnan(obs_et_wecann):
            ax.axhline(obs_et_wecann, color='blue', linestyle='--', linewidth=2.5,
                      alpha=0.8, label=f'WECANN ET: {obs_et_wecann:.2f}', zorder=2)
        if not np.isnan(obs_et_class_val):
